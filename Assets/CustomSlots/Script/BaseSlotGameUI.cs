...
namespace CSFramework {
	...
	public class BaseSlotGameUI : MonoBehaviour {
		...
		public List<float> betList = new List<float>() {0.1f, 0.2f, 1};
		...
		*/
		...
		public virtual void RefreshRoundCost() { textRoundCost.text = "" + string.Format("{0:0.00}", slot.gameInfo.roundCost); }
		public virtual void RefreshMoney() { textMoney.text = "" + string.Format("{0:0.00}", slot.gameInfo.balance); }
		public virtual void RefreshBet() { textBet.text = "" + string.Format("{0:0.00}", slot.gameInfo.bet);  }
		...
		public virtual void ToggleFreeSpin(bool enable) 
        { //goFreeSpin.SetActive(enable); 
            slot.gameInfo.roundsCompleted = 0;
        }
		...
		public virtual bool SetBet(int index) {
            if(index >= betList.Count)
            {
                index = 0;
            }

            if (!slot.isIdle || index < 0) return false;
