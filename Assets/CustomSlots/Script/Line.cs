...
namespace CSFramework {
	...
	public class Line : MonoBehaviour, IComparable {
        ...
        // Added By VC
        public Sprite mOnSprite, mOffSprite;
		...
		public virtual void EffectSwitchLine(bool? enable, bool instant) {
            ...
            DOTween.Complete(image_Right);
            image.color = Color.white;
            image_Right.color = Color.white;

            if (enable ?? isLineEnabled) {
                image.sprite = mOnSprite;
                image_Right.sprite = mOnSprite;
                ...
                image.sprite = mOffSprite;
                image_Right.sprite = mOffSprite;

                //v     if (instant) cg.alpha = 0.5f;
                //v	else cg.DOFade(0.5f, 0.3f);
            }
        }
		...
		/// <returns></returns>
		public virtual Tweener DrawPath(float duration, int _Index, Ease ease = Ease.InFlash) {
			...
			Color color = SlotManager.SlotManagerInstance.mLineColors[_Index]; //gradientColor;
			color.a = 1f;

			sgl.color = new Color(0, 0, 0, 0);
			sgl.transform.SetParent(lineManager.transform.parent);
            sgl.transform.SetSiblingIndex(lineManager.transform.GetSiblingIndex() + 1);
