...
namespace CSFramework {
	...
	public class SlotEffectManager {
		...
		public class SymbolHitEffect : IComparable {
			...
			public Sequence Play(SymbolHolder holder, int orderInLine, int _LineIndex, bool _IsScatter = false) {
				...
				sequence.Join(holder.HighlightBorder(duration, _LineIndex, _IsScatter));
		...
		public class LineHitEffect {
			...
			public Sequence Play(Line line, float duration) {
                Sequence sequence = Util.Sequence();
                if (iconScale != Vector3.one)
                {
                    sequence.Join(line.image.transform.DOScale(iconScale, duration*0.5f).SetLoops(2, LoopType.Yoyo).SetEase(iconScaleEase));
                    sequence.Join(line.image_Right.transform.DOScale(iconScale, duration * 0.5f).SetLoops(2, LoopType.Yoyo).SetEase(iconScaleEase));
                }
				Color clr = SlotManager.SlotManagerInstance.mLineColors[line.order];
				clr.a = 1f;

				sequence.Join(line.image.DOColor(clr, duration*0.5f).SetEase(iconColorEase).SetLoops(2, LoopType.Yoyo));

              //  sequence.Join(line.image.DOColor(line.gradientColor, duration*0.5f).SetEase(iconColorEase).SetLoops(2, LoopType.Yoyo));
				if (drawLine) sequence.Join(line.DrawPath(duration, line.order, pathColorEase));
				return sequence;
			}
		}
		...
		//v		line.HighlightLineIcon(iconDuration, iconColorEase);
		//v		line.DrawPath(pathDuration, pathColorEase);
			}
		}
