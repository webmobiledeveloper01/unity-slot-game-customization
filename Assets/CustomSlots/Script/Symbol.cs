...
namespace CSFramework {
	...
	public class Symbol : MonoBehaviour {
		...
		[HideInInspector] public float[] pays;
		...
		public virtual bool CanMatch(Symbol symbol) {
			...
			return symbol == this || symbol.matchType == MatchType.Wild || symbol.matchType == MatchType.WildScatter || this.matchType == MatchType.Wild || this.matchType == MatchType.WildScatter;
		}
		...
		/// <returns></returns>
		public virtual float GetPayAmount(int chains) {
			chains--;
			return chains >= pays.Length ? pays[pays.Length - 1] : pays[chains];
		}

		internal float GetMaxPay() { return pays.Length > 0 ? pays[pays.Length - 1] : 0; }

		internal void Validate() {
			pays = Util.StringToFloats(_pays);
			minChains = -1;
