...
namespace CSFramework {
	...
	public class SymbolGenLog : SymbolLog {
		...
		public float balance { get { return income - totalCost; } }
		...
		public void ProcessResult() {
			...
			if (gen.setting.sortMode == SymbolGen.SortMode.Profit) symbolLogs.Sort((x, y) => (int) (y.income - x.income));
	...
	public class SymbolLog {
		...
		public string Ratio(float a, float b) { return "" + Math.Round((float) a/b, 2); }

		public string Percentage(float a, float b, int round = 0) {
			if (round > 0) return "" + Math.Round(100f*a/b, round) + "%";
			return "" + a*100/b + "%";
		}

		public void LogHit(HitInfo info) {
			float pay = info.hitSymbol.GetPayAmount(info.hitChains);

			if (info.hitSymbol.payType == Symbol.PayType.Normal) income += pay;
			else if (info.hitSymbol.payType == Symbol.PayType.FreesSpin) {
				if (!symbol) {
					freeSpins += 10;
					gen.gameInfo.freeSpins += 10;
					if (gen.gameInfo.freeSpins > 300)
					{
						gen.gameInfo.freeSpins = 300;
					}
				}
