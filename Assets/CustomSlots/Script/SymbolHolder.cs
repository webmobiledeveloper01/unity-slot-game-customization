...
namespace CSFramework {
	...
	public class SymbolHolder : MonoBehaviour {
		...
		public void RefreshImage() {
            ...
            //v	if (animator) animator.runtimeAnimatorController = symbol.animator ? symbol.animator.runtimeAnimatorController : null;
            }
		}
		...
		/// <returns></returns>
		public virtual Tweener HighlightBorder(float duration,int _LineIndex, bool _IsScatter = false) {
			if (animator)
			{
			//	if (_IsScatter)
				{
			//		animator.runtimeAnimatorController = SlotManager.SlotManagerInstance.mBonusSpinAnimator.runtimeAnimatorController;
				}
			//	else
				{ 
					animator.runtimeAnimatorController = symbol.animator ? symbol.animator.runtimeAnimatorController : null;
				}
			} 
			...
			if (_LineIndex > -1)
			{
				color = SlotManager.SlotManagerInstance.mLineColors[_LineIndex];
			}
			else
			{
				color = Color.red;
			...
			return border.DOColor(color, duration*0.5f).SetEase(Ease.OutCubic).SetLoops(2, LoopType.Yoyo).OnComplete(() => {
